...
{
    return PipelineRegistry::registerShader(pipelineResource, name, shaderFileName, entryPoint).Get() != nullptr;
}
...
bool PipelineResourceManager::buildPSO(
    ...
    const std::string& name)
{
    return PipelineRegistry::registerPSO(pipelineResource, name, device, rootSig, desc).Get() != nullptr;
}
    ...
    const std::string& entryPoint)
{
    ...
    shaderBlob = D3D12Utility::CompileShaderOnline(AnsiToWstring(shaderFileName), nullptr, entryPoint, "vs_5_1");
