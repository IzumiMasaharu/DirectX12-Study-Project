...
RenderRuntime::RenderRuntime(HINSTANCE hInstance) : D3D12RenderHost(hInstance)
{
...
// 消息过程处理函数
LRESULT RenderRuntime::wndMsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_ACTIVATE:
	...
	default:
		return D3D12RenderHost::wndMsgProc(hwnd, msg, wParam, lParam);
	}
}

// 应用程序初始化
bool RenderRuntime::Initialize()
{
	...
	if (!D3D12RenderHost::InitializeD3D12Device())
		return false;
	Resize();

	ThrowIfFailed(commandList->Reset(commandAllocator.Get(), nullptr));
	renderSourceManager.initialize(d3dDevice.Get(), commandList.Get());
	BuildRootSignature();
	BuildShaders();
	BuildInputLayout();

	if (sceneSetupCallback)
		sceneSetupCallback(*this);

	sceneManager.buildRenderQueue(camera.getPositionFloat3());
	allRenderItems = sceneManager.getSortedRenderItems();
	BuildDescriptorHeaps();
	BuildFrameResources();
	resourceBindingReady = true;
	BuildPSOs();
	...
	if (startThreads)
	{
		controlThread = std::thread(&RenderRuntime::ControlLoop, this);
		renderThread = std::thread(&RenderRuntime::RenderLoop, this);
	}

	return true;
}

void RenderRuntime::ConfigureOwnedWindow(UINT width, UINT height, const std::wstring& title)
{
	useExternalWindow = false;
	wndDesc.clientWidth = width > 0 ? static_cast<int>(width) : wndDesc.clientWidth;
	wndDesc.clientHeight = height > 0 ? static_cast<int>(height) : wndDesc.clientHeight;
	if (!title.empty())
	{
		ownedWindowTitle = title;
		wndDesc.title = ownedWindowTitle.c_str();
	}
	clientWidth = wndDesc.clientWidth;
	clientHeight = wndDesc.clientHeight;
}

void RenderRuntime::UseExternalWindow(HWND hwnd, UINT width, UINT height)
{
	useExternalWindow = true;
	mainWndHwnd = hwnd;

	RECT clientRect = {};
	if ((width == 0 || height == 0) && hwnd && GetClientRect(hwnd, &clientRect))
	{
		width = static_cast<UINT>(clientRect.right - clientRect.left);
		height = static_cast<UINT>(clientRect.bottom - clientRect.top);
	}

	clientWidth = width > 0 ? static_cast<int>(width) : wndDesc.clientWidth;
	clientHeight = height > 0 ? static_cast<int>(height) : wndDesc.clientHeight;
	wndDesc.clientWidth = clientWidth;
	wndDesc.clientHeight = clientHeight;
}

void RenderRuntime::SetStartThreads(bool enabled)
{
	startThreads = enabled;
}

void RenderRuntime::SetShaderDirectory(const std::wstring& directory)
{
	if (directory.empty())
		return;

	shaderDirectory = directory;
	if (shaderDirectory.back() != L'/' && shaderDirectory.back() != L'\\')
		shaderDirectory += L"/";
}

void RenderRuntime::ResizeRenderTarget(UINT width, UINT height)
{
	if (width == 0 || height == 0)
		return;

	resizeInfo.newWidth = static_cast<int>(width);
	resizeInfo.newHeight = static_cast<int>(height);
	resizeInfo.isResized = true;
}

void RenderRuntime::RunResourceCommand(const std::function<void()>& command)
{
	if (!command)
		return;
	...
	const bool resumeRender = isRenderPaused == false;
	isRenderPaused = true;
...
void RenderRuntime::SetSceneSetup(SceneSetupCallback callback)
{
	sceneSetupCallback = std::move(callback);
}

Mayohoshi::CanvasHandle RenderRuntime::CreateCanvas(const Mayohoshi::CanvasDesc& desc)
{
	Mayohoshi::CanvasHandle handle;
	handle.name = desc.name.empty() ? "Screen" : desc.name;
	return handle;
}

Mayohoshi::TextureHandle RenderRuntime::CreateTexture(const Mayohoshi::TextureDesc& desc)
{
	if (renderSourceManager.createTexture(desc.name, desc.filePath) && resourceBindingReady)
	{
		Texture* texture = renderSourceManager.getTexture(desc.name);
		if (texture)
			srvDescriptorHeap.createTextureSrv(d3dDevice.Get(), *texture);
	}
	return { desc.name };
}

Mayohoshi::TextureSetHandle RenderRuntime::BindTextures(const Mayohoshi::TextureSetDesc& desc)
{
	TextureTableDesc tableDesc;
	tableDesc.name = desc.name;
	tableDesc.diffuseTexture = desc.diffuse.name;
	tableDesc.normalTexture = desc.normal.name;
	tableDesc.depthTexture = desc.depth.name;
	renderSourceManager.createTextureTable(tableDesc);
	return { desc.name };
}

Mayohoshi::MaterialHandle RenderRuntime::CreateMaterial(const Mayohoshi::MaterialDesc& desc)
{
	::MaterialDesc materialDesc;
	materialDesc.name = desc.name;
	materialDesc.albedo = desc.albedo;
	materialDesc.metallic = desc.metallic;
	materialDesc.roughness = desc.roughness;
	materialDesc.ior = desc.ior;
	materialDesc.emissive = desc.emissive;
	materialDesc.materialTransform = desc.materialTransform;
	materialDesc.renderLayer = desc.renderLayer;
	renderSourceManager.createMaterial(materialDesc);
	return { desc.name };
}

Mayohoshi::ModelHandle RenderRuntime::CreateCylinder(const Mayohoshi::CylinderDesc& desc)
{
	const std::string submeshName = desc.name + "_Submesh";
	renderSourceManager.createMeshGeometry(
		desc.name,
		false,
		{ SubmeshGeometryParams::Cylinder(submeshName, desc.bottomRadius, desc.topRadius, desc.height, desc.sliceCount, desc.stackCount) });
	return { desc.name, desc.name, submeshName, desc.layer };
}

Mayohoshi::ModelHandle RenderRuntime::CreateSphere(const Mayohoshi::SphereDesc& desc)
{
	const std::string submeshName = desc.name + "_Submesh";
	renderSourceManager.createMeshGeometry(
		desc.name,
		false,
		{ SubmeshGeometryParams::Sphere(submeshName, desc.radius, desc.sliceCount, desc.stackCount) });
	return { desc.name, desc.name, submeshName, desc.layer };
}

Mayohoshi::ModelHandle RenderRuntime::CreateGrid(const Mayohoshi::GridDesc& desc)
{
	const std::string submeshName = desc.name + "_Submesh";
	renderSourceManager.createMeshGeometry(
		desc.name,
		false,
		{ SubmeshGeometryParams::Grid(submeshName, desc.width, desc.depth, desc.m, desc.n) });
	return { desc.name, desc.name, submeshName, desc.layer };
}

Mayohoshi::ModelHandle RenderRuntime::CreateModel(const Mayohoshi::ObjModelDesc& desc)
{
	const std::string submeshName = desc.name + "_Submesh";
	renderSourceManager.createMeshGeometry(
		desc.name,
		desc.use32BitIndices,
		{ SubmeshGeometryParams::ImportedOBJ(submeshName, desc.filePath) });
	return { desc.name, desc.name, submeshName, desc.layer };
}

Mayohoshi::RenderObjectHandle RenderRuntime::AddInstance(const Mayohoshi::DrawDesc& desc)
{
	Mayohoshi::InstanceBatchDesc batchDesc;
	batchDesc.name = desc.name;
	batchDesc.model = desc.model;
	batchDesc.instances.push_back(
		{
			desc.transform,
			desc.material,
			desc.textures,
	...
	return AddInstances(batchDesc);
}

Mayohoshi::RenderObjectHandle RenderRuntime::AddInstances(const Mayohoshi::InstanceBatchDesc& desc)
{
	std::lock_guard<std::mutex> lock(sceneMutex);

	RenderItem* item = sceneManager.getRenderItem(desc.name);
	if (!item)
	{
		RenderItemDesc itemDesc;
		itemDesc.name = desc.name;
		itemDesc.geoName = desc.model.geometryName;
		itemDesc.submeshName = desc.model.submeshName;
		itemDesc.renderLayer = desc.model.layer;
		item = sceneManager.createRenderItem(itemDesc, renderSourceManager);
	}
	...
	if (item)
	{
		for (const auto& instance : desc.instances)
		{
			...
			instanceDesc.worldTransform = instance.transform.world;
			instanceDesc.textureTransform = instance.transform.textureTransform;
			instanceDesc.materialName = instance.material.name;
			instanceDesc.textureTableName = instance.textures.name;
			instanceDesc.textureFlags = instance.textureFlags;
			sceneManager.addInstance(desc.name, instanceDesc, renderSourceManager);
		}
	}
...
Mayohoshi::RenderObjectHandle RenderRuntime::AddToRender(const Mayohoshi::DrawDesc& desc)
{
	return AddInstance(desc);
}

void RenderRuntime::DrawTo(const Mayohoshi::CanvasHandle& canvas)
{
	UNREFERENCED_PARAMETER(canvas);
}

void RenderRuntime::DrawToScreen(const Mayohoshi::CanvasHandle& canvas)
{
	UNREFERENCED_PARAMETER(canvas);
}

void RenderRuntime::SetAmbientLight(const DirectX::XMFLOAT3& color, float intensity)
{
...
Mayohoshi::LightHandle RenderRuntime::AddLight(const Mayohoshi::LightDesc& desc)
{
	std::lock_guard<std::mutex> lock(lightingMutex);

	Light light;
	light.rgbIntensity = desc.intensity;
	light.start = desc.start;
	light.direction = desc.direction;
	light.end = desc.end;
	light.position = desc.position;
	light.spotPower = desc.spotPower;

	std::vector<Light>* target = &directionalLights;
	switch (desc.type)
	{
	case Mayohoshi::LightType::Directional:
		target = &directionalLights;
		break;
	case Mayohoshi::LightType::Point:
		target = &pointLights;
		break;
	case Mayohoshi::LightType::Spot:
		target = &spotLights;
		break;
	}

	const uint32_t index = static_cast<uint32_t>(target->size());
	target->emplace_back(light);
	return { desc.type, index, desc.name };
}

bool RenderRuntime::SetLight(const Mayohoshi::LightHandle& light, const Mayohoshi::LightDesc& desc)
{
	std::lock_guard<std::mutex> lock(lightingMutex);
	...
	if (light.index >= target->size())
		return false;

	Light& current = (*target)[light.index];
	current.rgbIntensity = desc.intensity;
	current.start = desc.start;
	current.direction = desc.direction;
	current.end = desc.end;
	current.position = desc.position;
	current.spotPower = desc.spotPower;
	return true;
}

bool RenderRuntime::SetLightPosition(const Mayohoshi::LightHandle& light, const DirectX::XMFLOAT3& position)
{
	std::lock_guard<std::mutex> lock(lightingMutex);

	std::vector<Light>* target = nullptr;
	if (light.type == Mayohoshi::LightType::Point)
		target = &pointLights;
	else if (light.type == Mayohoshi::LightType::Spot)
		target = &spotLights;

	if (!target || light.index >= target->size())
		return false;

	(*target)[light.index].position = position;
	return true;
}

bool RenderRuntime::SetLightDirection(const Mayohoshi::LightHandle& light, const DirectX::XMFLOAT3& direction)
{
	std::lock_guard<std::mutex> lock(lightingMutex);

	std::vector<Light>* target = nullptr;
	if (light.type == Mayohoshi::LightType::Directional)
		target = &directionalLights;
	else if (light.type == Mayohoshi::LightType::Spot)
		target = &spotLights;
	...
	(*target)[light.index].direction = direction;
	return true;
}

bool RenderRuntime::SetLightIntensity(const Mayohoshi::LightHandle& light, const DirectX::XMFLOAT3& intensity)
{
	std::lock_guard<std::mutex> lock(lightingMutex);
	...
	(*target)[light.index].rgbIntensity = intensity;
	return true;
}

void RenderRuntime::ClearLights()
{
	std::lock_guard<std::mutex> lock(lightingMutex);
...
void RenderRuntime::SetUpdateCallback(Mayohoshi::UpdateCallback callback)
{
	std::lock_guard<std::mutex> lock(updateCallbackMutex);
	updateCallback = std::move(callback);
}

bool RenderRuntime::SetObjectTransform(const Mayohoshi::RenderObjectHandle& object, const Mayohoshi::TransformDesc& transform, size_t instanceIndex)
{
	std::lock_guard<std::mutex> lock(sceneMutex);

	const bool worldChanged = sceneManager.setInstanceTransform(object.name, instanceIndex, transform.world);
	const bool textureChanged = sceneManager.setInstanceTextureTransform(object.name, instanceIndex, transform.textureTransform);
	return worldChanged || textureChanged;
}

void RenderRuntime::SetCameraPosition(const DirectX::XMFLOAT3& position)
{
	camera.setPosition(position);
}

void RenderRuntime::LookAt(const DirectX::XMFLOAT3& target)
{
	camera.lookAt(target);
}

void RenderRuntime::SetWireframe(bool enabled)
{
	isWireframeEnabled = enabled;
}

// 渲染线程循环
void RenderRuntime::RenderLoop()
{
	while (isRenderThreadRunning)
	{
		// 等待主线程资源更新完成
		std::unique_lock<std::mutex> lock(renderMutex);
		renderCV.wait(lock, [&] { return isFrameReady.load() || !isRenderThreadRunning.load(); });
...
// 窗口大小重新适配
void RenderRuntime::Resize()
{
	D3D12RenderHost::Resize();
...
// 更新帧画面
void RenderRuntime::Update(const GameTimer& GTimer)
{
	UpdateCameraState(GTimer);
	Mayohoshi::UpdateCallback callback;
	{
		std::lock_guard<std::mutex> lock(updateCallbackMutex);
		callback = updateCallback;
	}
	if (callback)
		callback(GTimer.TotalTime(), GTimer.DeltaTime());
...
// 绘制帧画面
void RenderRuntime::Draw(const GameTimer& GTimer)
{
	...
	std::array<ID3D12DescriptorHeap*, 1> descriptorHeaps = { srvDescriptorHeap.getHeap() };
	...
	auto textureTableBuffer = currentFrameResource->textureTableBuffer->Resource();
	commandList->SetGraphicsRootShaderResourceView(1, textureTableBuffer->GetGPUVirtualAddress());
	auto instanceStructuredBuffer = currentFrameResource->instanceStructuredBuffer->Resource();
	commandList->SetGraphicsRootShaderResourceView(2, instanceStructuredBuffer->GetGPUVirtualAddress());

	CD3DX12_GPU_DESCRIPTOR_HANDLE texDescriptor(srvDescriptorHeap.gpuStart());
	commandList->SetGraphicsRootDescriptorTable(4, texDescriptor);
	Texture* skycube = renderSourceManager.getTexture("skycube");
	if (skycube)
		commandList->SetGraphicsRootDescriptorTable(5, srvDescriptorHeap.gpuHandle(skycube->srvHeapIndex));

	{
		std::lock_guard<std::mutex> lock(sceneMutex);
		DrawRenderItems(commandList.Get(), sceneManager.getRenderItemsByLayer(RenderLayers::Opaque));
		commandList->SetPipelineState(PSOs["SkyCube"].Get());
		DrawRenderItems(commandList.Get(), sceneManager.getRenderItemsByLayer(RenderLayers::Skybox));
		commandList->SetPipelineState(PSOs["Wave"].Get());
		DrawRenderItems(commandList.Get(), sceneManager.getRenderItemsByLayer(RenderLayers::Transparent));
	}
...
// 处理键盘输入
void RenderRuntime::KeyboardMsgProc(UINT vk, bool pressed)
{
	keyDown[vk] = pressed;
...
// 当鼠标按下时调用
void RenderRuntime::MouseDown(WPARAM ButtonState, int x, int y)
{
...
// 当鼠标抬起时调用
void RenderRuntime::MouseUp(WPARAM ButtonState, int x, int y)
{
	ReleaseCapture();
}
// 当鼠标移动时调用
void RenderRuntime::MouseMove(WPARAM ButtonState, int x, int y)
{
...
// 载入纹理
void RenderRuntime::LoadTexture()
{
	UINT srvIndex = 0;
...
// 创建根签名
void RenderRuntime::BuildRootSignature()
{
	// 描述符范围，一段连续、类型相同的描述符
	CD3DX12_DESCRIPTOR_RANGE textureSrvRange;
	textureSrvRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1024, 0, 0);
	...
	auto staticSamplers = D3D12Utility::GetStaticSamplers();
...
void RenderRuntime::BuildDescriptorHeaps()
{
	const UINT textureCount = static_cast<UINT>(std::max<size_t>(1024, renderSourceManager.getTextureCount()));
	srvDescriptorHeap.initialize(d3dDevice.Get(), textureCount);
	renderSourceManager.forEachTexture(
...
// 编译着色器
void RenderRuntime::BuildShaders()
{
	shaders["VS"] = D3D12Utility::CompileShaderOnline(shaderDirectory + L"Vertex_Common.hlsl", nullptr, "VS", "vs_5_1");
	shaders["PS"] = D3D12Utility::CompileShaderOnline(shaderDirectory + L"Fragment.hlsl", nullptr, "PS", "ps_5_1");
	shaders["VS_Wave"] = D3D12Utility::CompileShaderOnline(shaderDirectory + L"Vertex_AniWave.hlsl", nullptr, "WaveVS", "vs_5_1");
	shaders["VS_Skycube"] = D3D12Utility::CompileShaderOnline(shaderDirectory + L"Skycube.hlsl", nullptr, "SkycubeVS", "vs_5_1");
	shaders["PS_Skycube"] = D3D12Utility::CompileShaderOnline(shaderDirectory + L"Skycube.hlsl", nullptr, "SkycubePS", "ps_5_1");
}
// 创建输入布局
void RenderRuntime::BuildInputLayout()
{
	inputLayout["Default"] =
	{
...
// 创建网格体
void RenderRuntime::BuildMeshGeometry()
{
	...
	Geo->vertexBufferGPU = D3D12Utility::CreateDefaultBuffer(d3dDevice.Get(), commandList.Get(), vertices.data(), vertexBufferByteSize, Geo->vertexBufferUploader);
	Geo->indexBufferGPU = D3D12Utility::CreateDefaultBuffer(d3dDevice.Get(), commandList.Get(), indices.data(), indexBufferByteSize, Geo->indexBufferUploader);
...
void RenderRuntime::BuildImportedGeometryFromOBJ()
{
	...
	geo->vertexBufferGPU = D3D12Utility::CreateDefaultBuffer(d3dDevice.Get(), commandList.Get(), vertices.data(), vbByteSize, geo->vertexBufferUploader);
	geo->indexBufferGPU = D3D12Utility::CreateDefaultBuffer(d3dDevice.Get(), commandList.Get(), indices.data(), ibByteSize, geo->indexBufferUploader);
...
// 创建材质
void RenderRuntime::BuildMaterials()
{
...
// 创建结构化材质常量缓冲区
void RenderRuntime::updateMaterialBuffers()
{
	auto currentMaterialStructuredBuffer = currentFrameResource->materialStructuredBuffer.get();

	renderSourceManager.forEachMaterial(
		[currentMaterialStructuredBuffer](Material* mat)
		{
		if (mat->numDirtyFrames > 0)
		{
			...
			materialData.renderLayer = static_cast<float>(mat->renderLayer);
...
void RenderRuntime::updateTextureTableBuffers()
{
	auto currentTextureTableBuffer = currentFrameResource->textureTableBuffer.get();

	renderSourceManager.forEachTextureTable(
		[currentTextureTableBuffer](UINT index, const TextureTable& table)
		{
			currentTextureTableBuffer->CopyData(index, table);
		});
}

// 创建渲染项
void RenderRuntime::BuildRenderItems()
{
...
void RenderRuntime::buildInstances()
{
	// Scene instances are now authored through the public Mayohoshi API.
}
// 创建帧资源
void RenderRuntime::BuildFrameResources()
{
	const UINT instanceCount = static_cast<UINT>(std::max<size_t>(4096, sceneManager.getTotalInstanceCount()));
	const UINT materialCount = static_cast<UINT>(std::max<size_t>(1024, renderSourceManager.getMaterialCount()));
	const UINT textureTableCount = static_cast<UINT>(std::max<size_t>(1024, renderSourceManager.getTextureTableCount()));

	for (int i = 0; i < gNumFrameResources; ++i)
		frameResources.emplace_back(std::make_unique<FrameResource>(d3dDevice.Get(), 1, instanceCount, materialCount, textureTableCount));
}
// 创建渲染管线状态对象
void RenderRuntime::BuildPSOs()
{
...
void RenderRuntime::UpdateCameraState(const GameTimer& GTimer)
{
...
// 更新物体常量缓冲区
void RenderRuntime::updateInstanceDataBuffers()
{
	...
	for (auto& it : allRenderItems)
	{
		...
		for (size_t i = 0; i < it->instances.size(); ++i)
		{
			...
			XMMATRIX normalMatrix = MathUtilities::InverseTranspose(worldTransform);
			...
			XMStoreFloat4x4(&instanceData.normalMatrix, XMMatrixTranspose(normalMatrix));
...
// 更新渲染过程常量
void RenderRuntime::updatePassConstBuffers()
{
	...
	{
		std::lock_guard<std::mutex> lock(lightingMutex);
		mRenderingPassConstantsBuffer.ambientIlluminating = ambientLight;

		UINT lightIndex = 0;
		const auto appendLights = [&mRenderingPassConstantsBuffer, &lightIndex](const std::vector<Light>& source)
			{
				UINT copied = 0;
				for (const auto& light : source)
				{
					...
					mRenderingPassConstantsBuffer.lights[lightIndex++] = light;
					++copied;
				}
				return copied;
			};

		mRenderingPassConstantsBuffer.directionalLightCount = appendLights(directionalLights);
		mRenderingPassConstantsBuffer.pointLightCount = appendLights(pointLights);
		mRenderingPassConstantsBuffer.spotLightCount = appendLights(spotLights);
	}
...
// 绘制渲染项
void RenderRuntime::DrawRenderItems(ID3D12GraphicsCommandList* commandList, const std::vector<RenderItem*>& renderItems)const
{
	auto instanceStructuredBuffer = currentFrameResource->instanceStructuredBuffer->Resource();
	...
	for (const auto item : renderItems)
	{
		...
		commandList->SetGraphicsRootShaderResourceView(
			2,
			instanceStructuredBuffer->GetGPUVirtualAddress() + static_cast<UINT64>(item->instanceBufferOffset) * sizeof(InstanceData));

		commandList->DrawIndexedInstanced(
			item->indexCount,
			static_cast<UINT>(item->instances.size()),
			item->indexStartLocation,
			item->vertexBaseLocation,
			0);
	}
}

// 获取指向MyApp类自身的指针
const RenderRuntime* RenderRuntime::getAppPtr()const
{
	return this;
}
