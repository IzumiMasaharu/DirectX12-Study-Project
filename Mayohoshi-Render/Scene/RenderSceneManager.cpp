...
bool RenderSceneManager::addInstance(
    const std::string& renderItemName,
    ...
    const RenderResourceManager& resourceManager)
{
    ...
    Material* mat = resourceManager.getMaterial(instanceDesc.materialName);
    if (!mat)
        return false;

    InstanceData data;
    data.worldTransform = instanceDesc.worldTransform;
    data.textureTransform = instanceDesc.textureTransform;
    data.materialIndex = mat->materialIndex;
    data.textureTableIndex = resourceManager.getTextureTableIndex(instanceDesc.textureTableName);
    data.textureFlags = instanceDesc.textureFlags;
...
bool RenderSceneManager::setInstanceTransform(
    const std::string& renderItemName,
    size_t instanceIndex,
    const DirectX::XMFLOAT4X4& worldTransform)
{
    RenderItem* item = getRenderItem(renderItemName);
    if (!item || instanceIndex >= item->instances.size())
        return false;

    item->instances[instanceIndex].worldTransform = worldTransform;
    item->numDirtyFrames = gNumFrameResources;
    return true;
}

bool RenderSceneManager::setInstanceTextureTransform(
    const std::string& renderItemName,
    size_t instanceIndex,
    const DirectX::XMFLOAT4X4& textureTransform)
{
    RenderItem* item = getRenderItem(renderItemName);
    if (!item || instanceIndex >= item->instances.size())
        return false;

    item->instances[instanceIndex].textureTransform = textureTransform;
    item->numDirtyFrames = gNumFrameResources;
    return true;
}
...
const std::vector<RenderItem*>& RenderSceneManager::getRenderItemsByLayer(RenderLayer layer) const
{
    ...
    {
        static const std::vector<RenderItem*> empty;
        return empty;
    }
    return it->second;
}
