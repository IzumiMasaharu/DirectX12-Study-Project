...
{
	if (mStopped)
		return static_cast<float>((mStopTime - mStartTime - mTotalPausdTime) * mSecondsPerCount);
	else
		return static_cast<float>((mCurrentTime - mStartTime - mTotalPausdTime) * mSecondsPerCount);
}
